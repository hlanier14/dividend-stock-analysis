...
def main(request):
    ...
    dividend_data_job = bg_client.query(f"SELECT * FROM `{DIVIDEND_TABLE_ID}` WHERE ticker IN UNNEST({tickers}) AND date >= DATE_SUB(CURRENT_DATE, INTERVAL 5 YEAR) ORDER BY date;")
